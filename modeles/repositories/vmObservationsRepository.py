...
def getTaxonsCommunes(insee):
    return session.query(distinct(VmTaxons.lb_nom), func.count(VmObservations.id_synthese).label('count'),VmTaxons.nom_vern)\
    .join(VmObservations, VmTaxons.cd_ref==VmObservations.cd_ref).group_by(VmTaxons.lb_nom, VmTaxons.nom_vern)\
