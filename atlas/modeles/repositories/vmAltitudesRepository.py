...
def getAltitudesChilds(cd_ref):
    ...
    altitude_labels = (
        db.session.query(TBibAltitudes.label_altitude).order_by(TBibAltitudes.altitude_min).all()
    )
    ...
    result = db.session.query(*sum_columns).filter(VmAltitudes.cd_ref.in_(all_ids)).one()
