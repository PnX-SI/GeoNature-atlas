...
def firstObservationChild(connection, cd_ref):
    sql = """
        ...
        FROM atlas.vm_taxons AS taxons
            JOIN atlas.vm_taxref AS taxref
...
def lastObservations(connection, mylimit, idPhoto):
    sql = """
        ...
        FROM atlas.vm_observations AS obs
            ...
            LEFT JOIN atlas.vm_medias AS medias
        ...
        if cached_stats != 2:
            ...
            sql = f"""
                ...
                FROM atlas.vm_taxons AS t
                    JOIN atlas.vm_observations AS o
...
def getLastDiscoveries(connection):
    sql="""
        SELECT DATE(MIN(dateobs)), vo.cd_ref, vt.lb_nom, vt.nom_vern,
            m.id_media, m.chemin, m.url, vt.group2_inpn
        FROM atlas.vm_observations AS vo
            JOIN atlas.vm_taxref AS vt
                ON vo.cd_ref = vt.cd_nom
            LEFT JOIN atlas.vm_medias AS m
                ON m.cd_ref = vo.cd_ref AND m.id_type = :thisidtype
        WHERE id_rang = 'ES'
