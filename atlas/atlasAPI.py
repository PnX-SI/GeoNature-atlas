...
def getObservationsMailleLastObsAPI(cd_ref):
    connection = utils.engine.connect()
    observations = vmObservationsMaillesRepository.getObservationsMaillesLastObsChilds(
        connection, cd_ref
    )
    connection.close()
    return jsonify(observations)
...
def getPhotosGroup(group):
    ...
    photos = vmMedias.getPhotosGalleryByGroup(
        connection, config.ATTR_MAIN_PHOTO, config.ATTR_OTHER_PHOTO, group
    )
    connection.close()
...
def getPhotosGallery():
    ...
    photos = vmMedias.getPhotosGallery(
        connection, config.ATTR_MAIN_PHOTO, config.ATTR_OTHER_PHOTO
    )
    connection.close()
    return jsonify(photos)
