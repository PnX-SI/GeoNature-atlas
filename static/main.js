...
autocompleteSearch = function(inputID, urlDestination, nbProposal){
  $(inputID).autocomplete({
    source: function (request, response) {
      ...
      $(inputID).attr('loading', 'true').css("background-image", "url('" + configuration.URL_APPLICATION + "/static/images/loading3.gif')");
      $.get(configuration.URL_APPLICATION + searchUrl, {search:request.term, limit: nbProposal}, function(results) {
        response(results.slice(0, nbProposal));
        $(inputID).attr('loading', 'false').css("background-image", "none");
      });
    },
