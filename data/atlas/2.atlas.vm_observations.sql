...
CREATE MATERIALIZED VIEW atlas.vm_observations AS
    WITH centroid_synthese AS (
        ...
        -- Données sensible
        -- Si l'observation est une ligne ou un polygone elle peut intersecter
        -- plusieur fois le même type de zonage.
        SELECT DISTINCT ON (s.id_synthese, cor.type_code)
            s.id_synthese,
