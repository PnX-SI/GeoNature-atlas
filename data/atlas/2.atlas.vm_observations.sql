...
AS $function$
  ...
  BEGIN
    ...
    -- If nomenclature id is not found, return 0 to avoid NULL which
    -- causes problems when used in the IN clause.
    RETURN COALESCE(nomenclatureId, 0);
  END;
$function$ ;
...
-- View indexes:
CREATE UNIQUE INDEX pk_municipality_subdivided
  ON atlas.municipality_subdivided USING btree (gid);
CREATE INDEX idx_municipality_subdivided_geom
  ON atlas.municipality_subdivided USING gist (geom);
...
CREATE UNIQUE INDEX pk_observations_blurred
  ON atlas.observations_blurred USING btree (id_synthese);
...
CREATE UNIQUE INDEX i_observations_blurred_geometry
  ON atlas.observations_blurred_geometry USING btree (id_synthese, id_area);
...
CREATE UNIQUE INDEX pk_observations_blurred_centroid
  ON atlas.observations_blurred_centroid USING btree (id_synthese);
CREATE INDEX idx_observations_blurred_centroid
  ON atlas.observations_blurred_centroid USING gist (geom_point);
...
CREATE UNIQUE INDEX pk_observations_blurred_insee
  ON atlas.observations_blurred_insee USING btree (id_synthese);
...
-- View indexes:
CREATE INDEX index_gist_vm_observations_the_geom_point
  ON atlas.vm_observations USING gist (the_geom_point);
CREATE INDEX vm_observations_altitude_retenue_idx
  ON atlas.vm_observations USING btree (altitude_retenue);
CREATE INDEX vm_observations_cd_ref_idx
  ON atlas.vm_observations USING btree (cd_ref);
CREATE INDEX vm_observations_dateobs_idx
  ON atlas.vm_observations USING btree (dateobs);
CREATE UNIQUE INDEX vm_observations_id_observation_idx
  ON atlas.vm_observations USING btree (id_observation);
CREATE INDEX vm_observations_insee_idx
  ON atlas.vm_observations USING btree (insee);
