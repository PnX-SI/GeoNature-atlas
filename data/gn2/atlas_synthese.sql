...
CREATE OR REPLACE VIEW synthese.syntheseff AS
 WITH areas AS (
	SELECT sa.id_synthese, sa.id_area, a.centroid, st_transform(centroid, 3857) as centroid_3857, t.type_code
	...
	SELECT s.id_synthese,
	    ...
	    false AS supprime,
	    true AS diffusable, id_nomenclature_diffusion_level, cd_nomenclature
	  ...
	  WHERE (NOT dl.cd_nomenclature::text = '4'::text OR id_nomenclature_diffusion_level IS NULL)  AND NOT s.last_action = 'D'
)
